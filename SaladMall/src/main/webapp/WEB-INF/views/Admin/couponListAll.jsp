									...
									<tbody>
										<c:forEach var="cvo" items="${couponListAll }">
											<tr role="row" class="odd">
												<td class="sorting_1">
												<a href="/Admin/couponInfo?coupon_id=${cvo.coupon_id }&pageNum=${pm.cri.pageNum}">${cvo.coupon_id }</a></td>
...
<div class="text-center">
	<ul class="pagination">	
		<!-- 이전 -->
		<c:if test="${pm.prev }">
			<li><a href="couponListAll?pageNum=${pm.startPage-1 }"> &laquo;</a></li>
		</c:if>
		<!-- 페이지 번호 -->
		<c:forEach begin="${pm.startPage }" end="${pm.endPage }" var="idx">
			<li <c:out value="${pm.cri.pageNum == idx? 'class = active ':''}"/>>
				<a href="couponListAll?pageNum=${idx }">${idx }</a>
			</li>
		</c:forEach>
		<!-- 다음 -->
		<c:if test="${pm.next && pm.endPage > 0 }">
			<li><a href="couponListAll?pageNum=${pm.endPage + 1}"> &raquo;</a></li>
		</c:if>
	</ul>
</div>
